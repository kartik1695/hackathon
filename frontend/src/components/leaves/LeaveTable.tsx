...
function exportCSV(leaves: Leave[], isManager: boolean) {
  ...
  const rows = leaves.map(lv => isManager
    ? [lv.employee_name ?? "", TYPE_LABELS[lv.leave_type] ?? lv.leave_type, lv.from_date, lv.to_date, lv.days_count, lv.status, `"${(lv.reason ?? "").replace(/"/g, '""')}"`]
    : [TYPE_LABELS[lv.leave_type] ?? lv.leave_type, lv.from_date, lv.to_date, lv.days_count, lv.status, `"${(lv.reason ?? "").replace(/"/g, '""')}"`]
  );
...
export default function LeaveTable({ leaves, loading, isManager, onApprove, onReject, onCancel }: LeaveTableProps) {
  ...
  const filtered = useMemo(() => {
    return leaves.filter(lv => {
      ...
      if (nameFilter && !(lv.employee_name ?? "").toLowerCase().includes(nameFilter.toLowerCase())) return false;
      return true;
    });
  ...
  if (loading) {
    return (
      <div className="space-y-2">
        {[1, 2, 3].map(i => (
          <div key={i} className="h-16 rounded-2xl animate-pulse" style={{ background: "var(--primary-pale)" }} />
        ))}
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        {hasFilters && (
          <button
            onClick={() => { setFromDate(""); setToDate(""); setTypeFilter(""); setNameFilter(""); }}
        ...
        >
          ...
          Export CSV {filtered.length !== leaves.length && `(${filtered.length})`}
        </button>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="py-12 text-center">
          ...
          <p className="text-sm font-medium" style={{ color: "var(--text-muted)" }}>
            {hasFilters ? "No results match your filters" : "No leave requests found"}
          </p>
        </div>
      ) : (
        <div className="space-y-2">
          {filtered.map(lv => {
            const ss = STATUS_STYLES[lv.status] ?? { bg: "#F3F4F6", color: "#6B7280" };
            ...
            return (
              ...
              >
                ...
                <span
                  ...
                  style={{ background: "var(--primary-dark)", color: "white", minWidth: "60px", textAlign: "center" }}
                >
                  {typeLabel}
                </span>
                ...
                <div className="flex-1 min-w-0">
                  {isManager && lv.employee_name && (
                    <p className="text-xs font-bold mb-0.5" style={{ color: "var(--text-dark)" }}>{lv.employee_name}</p>
                  )}
                  <div className="flex items-center gap-2">
                    ...
                    {lv.is_half_day && (
                      <span className="text-[10px] px-1.5 py-0.5 rounded-full" style={{ background: "var(--primary-pale)", color: "var(--primary)" }}>
                        half day
                      </span>
                    )}
                  </div>
                  {lv.reason && <p className="text-xs truncate mt-0.5" style={{ color: "var(--text-muted)" }}>{lv.reason}</p>}
                </div>

                {/* Days */}
                <span className="text-sm font-bold flex-shrink-0" style={{ color: "var(--text-dark)" }}>
                  {lv.days_count}d
                </span>
