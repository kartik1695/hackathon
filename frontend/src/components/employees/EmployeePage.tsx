...
const AV_COLORS = ["#7c3aed","#2563eb","#0d9488","#e11d48","#d97706","#16a34a","#0ea5e9"];

function toDisplay(e: Employee, idx: number): EmpDisplay {
  ...
  return {
    ...
    joined: e.date_of_joining ? new Date(e.date_of_joining).toLocaleDateString("en-IN",{day:"numeric",month:"short",year:"numeric"}) : "—",
    status: e.is_active ? "active" : "inactive",
    av: name.split(" ").map((w: string) => w[0]).join("").toUpperCase().slice(0, 2),
...
const Av = ({ init, color, size = 34 }: { init: string; color: string; size?: number }) => (
  <div style={{
...
const Pill = ({ children, color, bg, dot = true, size = 10.5 }: {
  children: React.ReactNode; color: string; bg?: string; dot?: boolean; size?: number;
}) => (
  <span style={{
...
const Btn = ({ children, onClick, variant = "primary", small = false, icon, danger = false }: {
  children: React.ReactNode; onClick?: () => void; variant?: string;
  small?: boolean; icon?: string; danger?: boolean;
}) => {
  const bg = danger ? "var(--danger)" : variant === "primary" ? "var(--accent)" : "transparent";
  ...
  return (
    <button onClick={onClick} style={{
      ...
      whiteSpace: "nowrap", transition: "opacity 0.15s", fontFamily: "inherit",
    }}
...
const Modal = ({ title, onClose, children, width = 480 }: {
  ...
  <div style={{
    ...
    <div style={{
      background: "var(--card)", borderRadius: 20, padding: 28, width, maxWidth: "95vw",
      boxShadow: "0 24px 80px rgba(0,0,0,0.2)", border: "1px solid var(--cardBorder)",
      maxHeight: "90vh", overflowY: "auto",
    }} onClick={e => e.stopPropagation()}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <h2 style={{ fontSize: 18, fontWeight: 250, color: "var(--ink)", margin: 0, letterSpacing: "-0.01em" }}>{title}</h2>
        <button onClick={onClose} style={{
          width: 30, height: 30, borderRadius: 8, border: "1px solid var(--cardBorder)",
...
const Field = ({ label, value, onChange, type = "text", placeholder, options, required }: {
  ...
  type?: string; placeholder?: string; options?: string[]; required?: boolean;
}) => (
  <div style={{ marginBottom: 14 }}>
    ...
    {options ? (
      <select value={value} onChange={e => onChange(e.target.value)} style={{
        width: "100%", padding: "9px 12px", borderRadius: 10, border: "1px solid var(--cardBorder)",
...
interface OrgTreeNode { emp: EmpDisplay; children: OrgTreeNode[]; }
...
function EmpCard({ emp, onView }: { emp: EmpDisplay; onView: (e: EmpDisplay) => void }) {
  return (
    <div
      onClick={() => onView(emp)}
      style={{
        background: "var(--card)", border: "1px solid var(--cardBorder)", borderRadius: 14,
        padding: "12px 14px", width: CARD_W, cursor: "pointer",
      ...
      onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "var(--cardShadow)"; }}
    >
      <Av init={emp.av} color={emp.col} size={34} />
      <div style={{ minWidth: 0 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: "var(--ink)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 126 }}>{emp.name}</div>
        <div style={{ fontSize: 10, color: "var(--muted)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 126, marginTop: 1 }}>{emp.role}</div>
        <div style={{ marginTop: 5 }}>
          <span style={{ fontSize: 9, fontWeight: 700, padding: "2px 6px", borderRadius: 999, background: "var(--accentLight)", color: "var(--accentText)", letterSpacing: "0.04em", textTransform: "uppercase" }}>{emp.dept}</span>
        </div>
      </div>
    </div>
  );
...
const OrgNode = ({ node, onView }: { node: OrgTreeNode; onView: (e: EmpDisplay) => void }) => {
  const { emp, children } = node;
  const hasChildren = children.length > 0;

  // Width of this entire subtree
  const myW = subtreeWidth(node);

  // For each child, compute cumulative left offset (relative to this node's left edge)
  // so we know where its center sits.
  let cursor = 0;
  ...
  return (
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: myW }}>
      ...
      {hasChildren && (
        <>
          {/* Stem down from card to horizontal rail */}
          <div style={{ width: 2, height: STEM_H, background: LINE }} />

          {/* Horizontal rail */}
          <div style={{ position: "relative", width: myW, height: 2, flexShrink: 0 }}>
            <div style={{
              position: "absolute",
              top: 0,
              ...
              height: 2,
              background: LINE,
              // single child: just a dot, but we skip rail and go straight down
          ...
          <div style={{ display: "flex", gap: H_GAP, alignItems: "flex-start" }}>
            {children.map(child => (
              <div key={child.emp.id} style={{ display: "flex", flexDirection: "column", alignItems: "center", width: subtreeWidth(child) }}>
                {/* Vertical stem from rail down to child card */}
                <div style={{ width: 2, height: STEM_H, background: LINE }} />
                <OrgNode node={child} onView={onView} />
              </div>
            ))}
          </div>
        </>
...
function buildTree(emps: EmpDisplay[]): OrgTreeNode[] {
  const byName = Object.fromEntries(emps.map(e => [e.name, { emp: e, children: [] as OrgTreeNode[] }]));
...
export default function EmployeePage({ token, role }: EmployeePageProps) {
  ...
  useEffect(() => { setPage(1); }, [debouncedQ, dept]);
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const raw: Employee[] = Array.isArray(data) ? data : (data.results ?? []);
          setTotalCount(Array.isArray(data) ? raw.length : (data.count ?? raw.length));
          setEmployees(raw.map(toDisplay));
        }
      } finally {
        setLoading(false);
      }
    ...
    load();
  }, [token, page, debouncedQ, dept]);

  const depts = ["All", ...Array.from(new Set(employees.map(e => e.dept).filter(d => d !== "—")))];
  const list = employees.filter(e =>
    (dept === "All" || e.dept === dept) &&
    e.name.toLowerCase().includes(debouncedQ.toLowerCase()),
  );
  const orgRoots = buildTree(list);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14, padding: "0 4px" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 250, color: "var(--ink)", letterSpacing: "-0.02em", margin: 0 }}>People</h1>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <div style={{
            ...
            background: "var(--card)", border: "1px solid var(--cardBorder)", borderRadius: 999,
          }}>
          ...
          {canAddEmployee && (
            <Btn icon="plus" onClick={() => setShowAdd(true)}>Add Employee</Btn>
          )}
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 9, flexWrap: "wrap" }}>
        <div style={{ flex: 1, minWidth: 180, position: "relative" }}>
          ...
          <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search employees..."
            style={{
              width: "100%", padding: "9px 12px 9px 36px", borderRadius: 999,
              border: "1px solid var(--cardBorder)", background: "var(--card)", color: "var(--ink)",
      ...
      {/* Org view — card grid matching HumanEdgeHRMS design */}
      {view === "org" ? (
        <Card style={{ padding: 20 }}>
          {loading ? (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 14 }}>
              {Array.from({ length: 10 }).map((_, i) => (
                <div key={i} style={{ height: 88, borderRadius: 14, background: "var(--accentLight)", opacity: 0.5 }} />
              ))}
            </div>
          ) : list.length === 0 ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 14 }}>
              {list.map(e => (
                <div
                  ...
                  style={{
                    background: "var(--card)", border: "1px solid var(--cardBorder)", borderRadius: 14,
                    padding: "14px 14px 12px", cursor: "pointer", display: "flex", alignItems: "center", gap: 12,
                    boxShadow: "var(--cardShadow)", transition: "transform 0.15s, box-shadow 0.15s",
                  }}
                  onMouseEnter={ev => { ev.currentTarget.style.transform = "translateY(-2px)"; ev.currentTarget.style.boxShadow = "0 8px 20px rgba(0,0,0,0.10)"; }}
                  onMouseLeave={ev => { ev.currentTarget.style.transform = "translateY(0)"; ev.currentTarget.style.boxShadow = "var(--cardShadow)"; }}
                >
                  <Av init={e.av} color={e.col} size={38} />
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: "var(--ink)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{e.name}</div>
                    <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 1, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{e.role}</div>
                    <div style={{ marginTop: 6 }}>
                      <span style={{
                        fontSize: 9, fontWeight: 700, padding: "2px 7px", borderRadius: 999,
                        background: "var(--accentLight)", color: "var(--accentText)",
        ...
        <Card>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "var(--surface2,var(--accentLight))" }}>
                {["Employee", "Role", "Department", "Manager", "Joined", "Status", "Actions"].map((h, i) => (
                  <th key={i} style={{
                    padding: "11px 16px", textAlign: "left", fontSize: 10.5, fontWeight: 600,
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={7} style={{ padding: 40, textAlign: "center", color: "var(--muted)", fontSize: 13 }}>Loading…</td>
                </tr>
              ) : list.length === 0 ? (
                <tr>
                  <td colSpan={7} style={{ padding: 40, textAlign: "center", color: "var(--muted)", fontSize: 13 }}>No employees found.</td>
                </tr>
              ) : list.map(e => (
                <tr key={e.id} style={{ borderBottom: "1px solid var(--cardBorder)", cursor: "pointer" }}
                  onMouseEnter={ev => (ev.currentTarget.style.background = "var(--surface2,var(--accentLight))")}
                ...
                >
                  ...
                  <td style={{ padding: "11px 16px" }}>
                    <Pill color="var(--accent)" bg="var(--accentLight)">{e.dept}</Pill>
                  </td>
                  ...
                  <td style={{ padding: "11px 16px" }}>
                    <Pill color={e.status === "active" ? "#10b981" : "var(--muted)"}>{e.status}</Pill>
                  </td>
                  <td style={{ padding: "11px 16px" }}>
                    <div style={{ display: "flex", gap: 6 }}>
                      <button onClick={() => setViewEmp(e)} style={{
          ...
          {totalPages > 1 && (
            <div style={{ padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", borderTop: "1px solid var(--cardBorder)" }}>
              <span style={{ fontSize: 12, color: "var(--muted)" }}>
                {`${(page - 1) * PAGE_SIZE + 1}–${Math.min(page * PAGE_SIZE, totalCount)} of ${totalCount}`}
              </span>
              <div style={{ display: "flex", gap: 4 }}>
                <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                  style={{ padding: "5px 12px", borderRadius: 999, border: "1px solid var(--cardBorder)", background: "var(--card)", color: "var(--muted)", fontSize: 12, cursor: "pointer", fontFamily: "inherit", opacity: page === 1 ? 0.35 : 1 }}>
                  ← Prev
                </button>
                <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages}
                  style={{ padding: "5px 12px", borderRadius: 999, border: "1px solid var(--cardBorder)", background: "var(--card)", color: "var(--muted)", fontSize: 12, cursor: "pointer", fontFamily: "inherit", opacity: page === totalPages ? 0.35 : 1 }}>
                  Next →
                </button>
              </div>
            </div>
      ...
      {showAdd && (
        <Modal title="Add Employee" onClose={() => setShowAdd(false)}>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 8 }}>
            ...
            <Btn onClick={addEmp} icon="plus">Add Employee</Btn>
          </div>
        </Modal>
      )}
      ...
      {viewEmp && (
        <Modal title="Employee Profile" onClose={() => setViewEmp(null)} width={520}>
          <div style={{
            ...
            padding: 16, background: "var(--surface2,var(--accentLight))", borderRadius: 14,
          }}>
            ...
            <div>
              ...
              <div style={{ marginTop: 6 }}>
                <Pill color={viewEmp.status === "active" ? "#10b981" : "var(--muted)"} size={11}>{viewEmp.status}</Pill>
              </div>
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {([
              ...
              ...(viewEmp.salary ? [["Annual CTC", `₹${(viewEmp.salary / 100000).toFixed(1)}L`]] : []),
              ["Email", viewEmp.email || "—"],
              ["Status", viewEmp.status],
            ] as [string, string][]).map(([k, v]) => (
              <div key={k} style={{ padding: "10px 12px", background: "var(--surface2,var(--accentLight))", borderRadius: 10 }}>
